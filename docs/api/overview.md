...
### Node.js
```bash
npm install @rapido/partner-api
```

```javascript
const RapidoAPI = require('@rapido/partner-api');

const client = new RapidoAPI({
...
### Python
```bash
pip install rapido-partner-api
```

```python
from rapido_partner_api import RapidoClient
...
```java
RapidoAPI client = new RapidoAPI.Builder()
    .apiKey(System.getenv("RAPIDO_PARTNER_API_KEY"))
...
Access your integration metrics through the Partner Portal:
...
- `user.token.expired` - Token expiration events
- `user.consent.revoked` - User revokes app permissions
- `api.rate_limit.exceeded` - Rate limit warnings
